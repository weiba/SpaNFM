...
class image_feature:
    ...
    def load_vit_model(
        ...
        if self.vit_type=='vit_b':
            vit_pretrained_model = models.vit_b_16(pretrained=True)
            vit_pretrained_model.to(self.device)
        return vit_pretrained_model
    ...
    def image_crop(
        ...
        with tqdm(total=len(adata),
            ...
            for imagerow, imagecol in zip(adata.obs["imagerow"], adata.obs["imagecol"]):
                ...
                tile.thumbnail((target_size, target_size)) ##### 
