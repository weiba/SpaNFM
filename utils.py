...
def cluster(adata,X,df_meta,n_domains,refined=False):
        ...
        return adata.obs["domain"].tolist(),ARI,NMI
...
def found_gene_count(adata,cell,gene):
    ...
    selected_cells = adata.obs['labels'] == cell_type
...
def found_bigest_gene_expression(adata):
    ...
    cell_types = adata.obs['labels']
    cell_type_avg_expression = defaultdict(list)

    for idx, cell_type in enumerate(cell_types):
        gene_expression = adata.X[idx, :]
        cell_type_avg_expression[cell_type].append(gene_expression)
